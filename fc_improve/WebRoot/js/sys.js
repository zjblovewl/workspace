...
( function($) {
	$( function() {
		myself.location = function(url) {
			location.href = top.basePath + url;
		};

		// 弹出信息窗口 title:标题 msgString:提示信息 msgType:信息类型
		// [error,info,question,warning]
		myself.msgShow = function(title, msgString, msgType) {
			$.messager.show({
				title:title,
				msg:msgString,
				timeout:3000,
		...
		myself.msgAlert = function(title, msgString, msgType){
			$.messager.alert(title, msgString, msgType);
		};
		
		myself.ajaxCheck = function(data){
			if('sessionout' == data)
			{//session失效
				alert('页面过期，请重新登录!');
				window.top.location.href= top.basePath;
				return false;
			}else if('nolimit' == data)
			{
				alert('非法请求，您没有访问权限!');
				return false;
			}
		...
		$.extend($.fn.validatebox.defaults.rules, {
			...
			letter : {
				...
				message : '只能是 数字、字母、英文句号组成'
			},
			link : {
				validator : function(value, param) {
					return /^[a-zA-Z0-9\.\/]+$/.test(value);
				},
				message : '只能是 数字、字母、英文句号、/ 组成'
			},
			idcard : {
				validator : function(value, param) {
					return idCard(value);
		...
		var idCard = function(value) {
			...
			var d, sum = 0, v = '10x98765432', w = [ 7, 9, 10, 5, 8, 4, 2, 1,
					6, 3, 7, 9, 10, 5, 8, 4, 2 ], a = '11,12,13,14,15,21,22,23,31,32,33,34,35,36,37,41,42,43,44,45,46,50,51,52,53,54,61,62,63,64,65,71,81,82,91';
			var re = number
			...
			return (re[2].length == 9 || number.charAt(17) == v
					.charAt(sum % 11));
		}
