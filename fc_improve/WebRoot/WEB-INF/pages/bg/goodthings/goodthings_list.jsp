	...
	$(function(){
		...
		function openSaveWindow(id)
		{
			$.window({
   		        ...
   		        title:(id==''?'新增':'修改')+"凡人好事",
   		        url:'goodThings/toSave.htm?m=${m}&id='+id,
   		        width:windowWidth,
   		        height:windowHeight,  
   		        onClose:function(){
   		        	closeHandler();
		...
		myself.openViewWindow = function(id)
		{
			$.window({
			        ...
			        title:"查看汶南好人",
			        url:'goodThings/view.htm?m=${m}&id='+id,
			        width:windowWidth,
			        height:windowHeight,  
			        onClose:function(){
			        	closeHandler();
...
function batchInsert(){
	...
	var ajax_option={
		url:"goodThings/batchInsert.htm",
		success:function(data){
			var a ;
			eval("a="+data);
...
<table id="dg" class="easyui-datagrid" title="DataGrid Complex Toolbar" style="width:700px;height:250px"
			data-options="pagination:true,rownumbers:true,striped:true,fit:true,fitColumns:true,title:false,border:false,singleSelect:true,url:'goodThings/list.htm',toolbar:'#tb'">
		<thead>
			<tr>
