	...
	$(function(){
		...
		function openSaveWindow(id)
		{
			$.window({
   		        ...
   		        title:(id==''?'新增':'修改')+"志愿服务",
   		        url:'volunteerService/toSave.htm?m=${m}&id='+id,
   		        width:windowWidth,
   		        height:windowHeight,  
   		        onClose:function(){
   		        	closeHandler();
		...
		$('#icon-remove').click(function(){
			...
			$.messager.confirm('删除提示', '确定要删除所选的记录吗?', function(r){
		        if (r){
		        	$.ajax({
		        		url:'volunteerService/delete.htm?m=${m}',
		        		data:'id='+row.id,		
		...
		myself.openViewWindow = function(id)
		{
			$.window({
			        ...
			        title:"查看志愿服务",
			        url:'volunteerService/view.htm?m=${m}&id='+id,
			        width:windowWidth,
			        height:windowHeight,  
			        onClose:function(){
			        	closeHandler();
...
function batchInsert(){
	...
	var ajax_option={
		url:"volunteerService/batchInsert.htm",
		success:function(data){
			var a ;
			eval("a="+data);
...
<table id="dg" class="easyui-datagrid" title="DataGrid Complex Toolbar" style="width:700px;height:250px"
			data-options="pagination:true,rownumbers:true,striped:true,fit:true,fitColumns:true,title:false,border:false,singleSelect:true,url:'volunteerService/list.htm',toolbar:'#tb'">
		<thead>
			<tr>
	...
	<div id="tb" style="padding:5px;height:auto">
		...
		<form name="searchform" method="post" action="" id ="searchform" style="padding-top:5px;">
			标题:
				<input class="easyui-validatebox" maxlength="32" type="text" name="title"  id="title"></input>
			<a id="submit-search" class="easyui-linkbutton" iconCls="icon-search">搜索</a>
			<a id="reset-search" onclick="document.getElementById('title').value=''" class="easyui-linkbutton" iconCls="icon-no">重置</a>
