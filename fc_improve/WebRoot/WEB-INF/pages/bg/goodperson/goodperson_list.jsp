	...
	$(function(){
		...
		function openSaveWindow(id)
		{
			$.window({
   		        ...
   		        title:(id==''?'新增':'修改')+"汶南好人",
   		        url:'goodpersion/toSave.htm?m=${m}&id='+id,
   		        width:windowWidth,
   		        height:windowHeight,  
   		        onClose:function(){
   		        	closeHandler();
		...
		$('#icon-remove').click(function(){
			...
			$.messager.confirm('删除提示', '确定要删除所选的记录吗?', function(r){
		        if (r){
		        	$.ajax({
		        		url:'goodpersion/delete.htm?m=${m}',
		        		data:'id='+row.id,		
		...
		myself.openViewWindow = function(id)
		{
			$.window({
			        ...
			        title:"查看汶南好人",
			        url:'goodpersion/view.htm?m=${m}&id='+id,
			        width:windowWidth,
			        height:windowHeight,  
			        onClose:function(){
			        	closeHandler();
...
function batchInsert(){
	...
	var ajax_option={
		url:"goodpersion/batchInsert.htm",
		success:function(data){
			var a ;
			eval("a="+data);
...
<table id="dg" class="easyui-datagrid" title="DataGrid Complex Toolbar" style="width:700px;height:250px"
			data-options="pagination:true,rownumbers:true,striped:true,fit:true,fitColumns:true,title:false,border:false,singleSelect:true,url:'goodpersion/list.htm',toolbar:'#tb'">
		<thead>
			<tr>
