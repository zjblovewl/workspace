	...
	$(function(){
		...
		function openSaveWindow(id)
		{
			$.window({
   		        ...
   		        title:(id==''?'新增':'修改')+"通讯录",
   		        url:'addrbook/toSave.htm?m=${m}&id='+id,
   		        width:windowWidth,
   		        height:windowHeight,  
   		        onClose:function(){
   		        	closeHandler();
		...
		$('#submit-search').click(function(){
			...
			queryParams.phoneNum = $('#phoneNum').val();
			$('#dg').datagrid('reload');
		});
		...
		myself.openViewWindow = function(id)
		{
			$.window({
			        ...
			        title:"查看通讯录",
			        url:'addrbook/view.htm?m=${m}&id='+id,
			        width:windowWidth,
			        height:windowHeight,  
			        onClose:function(){
			        	closeHandler();
...
<table id="dg" class="easyui-datagrid" title="DataGrid Complex Toolbar" style="width:700px;height:250px"
			data-options="pagination:true,rownumbers:true,striped:true,fit:true,fitColumns:true,title:false,border:false,singleSelect:true,url:'addrbook/list.htm',toolbar:'#tb'">
		<thead>
			<tr>
			  	<th data-options="field:'name',width:200">姓名</th>
			  	<th data-options="field:'department',width:200">部门</th>
			  	<th data-options="field:'phoneNum',width:200">手机号</th>
	...
	<div id="tb" style="padding:5px;height:auto">
		...
		<form name="searchform" method="post" action="" id ="searchform" style="padding-top:5px;">
			姓名:
				<input class="easyui-validatebox" maxlength="32" type="text" name="name"  id="name"></input>
			部门:
				<input class="easyui-validatebox" maxlength="128" type="text" name="department"  id="department"></input>
			手机号:
				<input class="easyui-validatebox" maxlength="64" type="text" name="phoneNum"  id="phoneNum"></input>
			<a id="submit-search" class="easyui-linkbutton" iconCls="icon-search">搜索</a>
			<button type="button" id="btn_exp" onclick="batchImport();" style="display:none">
			批量导入
			</button>			
		</form>
	</div>
